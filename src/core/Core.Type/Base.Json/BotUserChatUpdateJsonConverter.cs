...
internal sealed class BotUserChatUpdateJsonConverter : JsonConverter<BotUserChatUpdate>
{
    public override BotUserChatUpdate? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
    ...
    public override void Write(Utf8JsonWriter writer, BotUserChatUpdate value, JsonSerializerOptions options)
    {
        if (value is null)
        {
    ...
    private static BotUserChat? GetUserChat(BotUpdateJson update)
        =>
        update switch
        {
